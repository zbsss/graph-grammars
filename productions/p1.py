...
def P1(id):
    ...
    lower_left_vertix = find_lower_left_vertex(lower_layer_squares)  # find the edge lowest vertix on the left
    ...
    lower_right_x = lower_left_vertix.x + lower_graph_fragment_width  # define new graph fragment vertices coordinates
    lower_right_y = lower_left_vertix.y
    upper_left_x = lower_left_vertix.x
    upper_left_y = lower_left_vertix.y + lower_graph_fragment_width
    upper_right_x = lower_right_x
    upper_right_y = upper_left_y
    middle_x = lower_left_vertix.x + (lower_graph_fragment_width / 2)
    middle_y = lower_left_vertix.y + (lower_graph_fragment_width / 2)
    lower_right_vertex = find_vertex_with_coordinates_and_remove_duplicates(lower_right_x, lower_right_y, # define vertices and remove duplicate vertices for neighbouring squares
                                                                              lower_layer_squares)
    ...
    edges = [(upper_left_vertex.id, lower_left_vertix.id), (upper_left_vertex.id, upper_right_vertex.id),   # define edges (connections between vertices (their ids))
             (upper_left_vertex.id, middle_vertex.id),
             (lower_left_vertix.id, lower_right_vertex.id), (lower_left_vertix.id, middle_vertex.id),
    ...
    new_fragment = GraphFragment(lower_layer_squares,   # define new graph fragment on lower layer
                                [lower_left_vertix, lower_right_vertex, upper_left_vertex, upper_right_vertex,
    ...
    graph_fragment.middle_vertex.label = VertexLabel.i  # set upper graph fragment middle vertix label to i - occupied
