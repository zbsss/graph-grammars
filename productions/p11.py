...
def P11(id1, id2, id3):
    ...
    upper_left = sorted(sorted(graph_fragment_3.vertices, key=lambda v: -v.x), key=lambda v: -v.y)[0]
    upper_right = sorted(sorted(graph_fragment_3.vertices, key=lambda v: v.x), key=lambda v: -v.y)[0]

    lowest_left = sorted(sorted(graph_fragment_3.vertices, key=lambda v: v.x), key=lambda v: v.y)[0]
    ...
    graph_fragment_3.edges.append((lower_left.id, lowest_left.id))
    middle = lower_1[1]
